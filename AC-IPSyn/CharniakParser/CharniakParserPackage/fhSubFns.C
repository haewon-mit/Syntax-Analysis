...
{
  if(!tree) return 1;
  if(Term::fromInt(tree->term)->name() == "S1") return 1;
  int loc = 0;
  int sz = tree->size;
  ...
  for( ; ; loc++)
    {
      ...
      nxt = fhs[loc];
      ECString ntrm = Term::fromInt(nxt->term)->name();
      //cerr << "NN " << ntrm << endl;
      if(ntrm== "STOP")
        {
          FullHist* par = tree->back;
          //cerr << "NPT "<< par->term << endl;
          FullHist* parpar = par->back;
          //if(parpar) cerr << "PP " << parpar->term << endl;
          return is_effEnd(tree->back,tree);
        }
	//punctuation rules are Language specific 
      if(ntrm == ":" || ntrm == ".") return 1;
      if(ntrm != ",") return 0;
      loc++;
      if(loc == sz) return 0;
      nxt = fhs[loc];
      ntrm = Term::fromInt(nxt->term)->name();
      if(ntrm == "''") return 1;
      return 0;
    }
