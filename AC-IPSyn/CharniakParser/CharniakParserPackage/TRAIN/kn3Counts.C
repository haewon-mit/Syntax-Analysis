...
main(int argc, char *argv[])
{
   ...
   /*
   trainingString += "f0-20.unk10.txt";
   //trainingString += "train.txt";
   //trainingString += "test.bad";
   ifstream trainingStream(trainingString.c_str());
   if( !trainingStream ) error( "No training stream" );
   */
   if(prune)
     {
       string wlString(path);
       wlString += "wordList.txt";
       ifstream wlStream(wlString.c_str());
       assert(wlStream);
       while(true)
	 {
	   string nxtw;
