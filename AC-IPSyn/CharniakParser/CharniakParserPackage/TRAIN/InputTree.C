...
readParse(istream& is)
{
  ...
  if(temp != ")")
    {
      cerr << *this << endl;
      error("Should have closed paren here.");
    }
}
...
newParse(istream& is, int& strt, InputTree* par)
{
  ...
  for( ; ; )
      ...
      if(temp == "(")
	{
	  ...
	  //nextTree = fixNPBifNecessary(nextTree, trm);
	  /* this fixes double question marks, etc, found in Brown corpus */
	  ECString curw = nextTree->word();
	  if(lstTree && lstTree->word() == curw
	     && (curw == "?" || curw == ";" || curw == "!"))
	    {
	      delete nextTree;
	      strt--;
...
ccTree()
{
  ...
  int seenCC = 0;
  int numComs = 0;
  InputTreesIter iti = subTrees_.begin();
  for( ; iti != subTrees_.end() ; iti++)
    {
      ECString strm = (*iti)->term();
      if(strm == term_) numSame++;
      else if(strm == "CC") seenCC = 1;
      else if(strm == ",") numComs++;
      else if(scorePunctuation(strm)) continue;
      else return false;
    }
  if(term_ == "NP" && (numSame == 2 &&  !seenCC)) return false;
  if((numSame > 1 && (seenCC || numComs))) return true;
  return false;
}
...
ccChild()
{
  if(term_ != "S1") assert(parent_);
  if(!parent_) return false;
  if(parent_->term() != term_) return false;
  if(parent_->ccTree()) return true;
  return false;
}
