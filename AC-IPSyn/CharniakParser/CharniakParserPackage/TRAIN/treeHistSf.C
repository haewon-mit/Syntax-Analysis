...
tree_grandparent_head(TreeHist* treeh)
{
  ...
  if(!wi)
    {
      if(prune || Feat::Usage == PARSE) return -1;
      cerr << *tree << endl;
      assert(wi);
    }
  int ans = wi->toInt();
  ...
  for( ; ; subTreeIter++ )
    {
      ...
      assert(i < sz);
      const Term* trm = Term::get(subTree->term());
      if(subTree->term() == "``") sawOpen = true;
      if(subTree->term() == "''") sawOpen = false;
      i++;
    }
  if(sawOpen) return 0;
  else return 1;
  ...
  for( ; ; subTreeIter++ )
    {
      ...
      const Term* trm = Term::get(subTree->term());
      if(subTree->term() == "''") sawOpen = true;
      else if(subTree->term() == "``") sawOpen = false;
      i--;
    }
  if(sawOpen) return 0;
  else return 1;
